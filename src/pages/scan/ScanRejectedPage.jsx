...
export function ScanRejectedPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3"><Card className="p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Total Issues</p><p className="mt-1 text-xl font-black text-slate-950">{rows.length}</p></Card><Card className="p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Retake Requested</p><p className="mt-1 text-xl font-black text-slate-950">{rows.filter((r) => r.actionNeeded === 'Retake Requested').length}</p></Card><Card className="p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Rejected</p><p className="mt-1 text-xl font-black text-slate-950">{rows.filter((r) => r.actionNeeded === 'Rejected').length}</p></Card></div>
