  ...
  return (
    <Modal open={open} onClose={onClose} title={`Scan report · ${order.patient?.fullName}`} description="Save a draft first, then submit for radiologist review when findings and impression are complete.">
      <div className="space-y-4">
        <div className="rounded-2xl bg-slate-50 p-4"><p className="font-black text-slate-950">{order.id}</p><p className="text-sm text-slate-500">{describeOrderItems(order.items)} · {equipment || 'No equipment assigned'}</p></div>
        <FormField label="Image / DICOM upload"><input type="file" multiple accept=".dcm,.dicom,application/dicom,image/*,.pdf" className={inputClass} onChange={(event) => setFiles(Array.from(event.target.files || []).map(fileMeta))} help="DICOM-ready frontend accepts .dcm/.dicom, image and PDF files. This demo stores metadata; production will connect to PACS/DICOMweb storage." /></FormField>
        {files.length > 0 && <div className="grid gap-2 sm:grid-cols-2">{files.map((file) => <div key={file.name} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs font-bold text-slate-700"><div className="flex items-center justify-between gap-2"><span>{file.name}</span>{file.isDicom && <StatusBadge status="DICOM" />}</div><span className="font-semibold text-slate-400">{Math.ceil((file.size || 0) / 1024)} KB · {file.type || 'Unknown type'}</span>{file.isDicom && <p className="mt-1 text-[11px] font-semibold text-purple-700">Study/Series metadata will be indexed by backend PACS later.</p>}</div>)}</div>}
        ...
        <FormField label="Internal technician notes"><textarea rows="3" className={inputClass} value={internalNotes} onChange={(event) => setInternalNotes(event.target.value)} placeholder="QA notes not visible to doctor" /></FormField>
...
export function AcceptedScansPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4"><Card className="p-4"><Info label="Accepted Cases" value={accepted.length} /></Card><Card className="p-4"><Info label="Draft Reports" value={(data.results || []).filter((r) => r.department === 'Imaging' && r.status === 'Draft').length} /></Card><Card className="p-4"><Info label="Pending Review" value={(data.results || []).filter((r) => r.department === 'Imaging' && r.status === 'Pending Review').length} /></Card><Card className="p-4"><Info label="DICOM Ready" value=".dcm/.dicom" /></Card></div>
...
function Info({ label, value }) { return <div><p className="text-xs font-black uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 text-lg font-black text-slate-950">{value ?? '—'}</p></div>; }
